...
(function () {
  ...
  function renderRotation() {
    ...
    document.querySelectorAll('.court-cell').forEach(function (c) {
      c.onclick = function () {
        var p = c.dataset.player || null;
        matchState.selectedPlayer = p || null;
        document.querySelectorAll('.court-cell').forEach(function (x) { x.classList.remove('selected'); });
  ...
  function updateButtonVisibility() {
    ...
    var servicePass = isServicePass();
    var zoneButtonsDisabled = servicePass && !matchState.selectedPlayer;
    document.querySelectorAll('.btn-pass-zone').forEach(function (btn) {
      btn.disabled = zoneButtonsDisabled;
    });
    document.querySelectorAll('.btn-overpass').forEach(function (btn) {
      btn.disabled = zoneButtonsDisabled;
    });
  ...
  function givePoint(toHome) {
    ...
    if (setWon === 'home') {
      ...
      window.location.href = 'index.php?mode=setLineup&set=' + (matchState.currentSetIndex + 1);
      return;
    } else if (setWon === 'away') {
      matchState.awaySets++;
      matchState.currentSetIndex++;
  ...
  document.querySelectorAll('.btn-event').forEach(btn => {
    btn.addEventListener('click', async () => {
      if (btn.classList.contains('btn-outcome-sub')) return;
      const desc = btn.getAttribute('data-desc');
      const short = btn.getAttribute('data-short') || '';
      const panel = parseInt(btn.getAttribute('data-panel'), 10);
      const buttonTeam = btn.getAttribute('data-team');
      ...
      const team = teamForEvent(panel, buttonTeam);
  ...
  document.querySelectorAll('.btn-outcome-sub').forEach(btn => {
  ...
  document.getElementById('pointHome').addEventListener('click', function () {
    showOutcomeSub(true);
  });
  document.getElementById('pointAway').addEventListener('click', function () {
    showOutcomeSub(false);
  });
  document.getElementById('outcomeSubCancel').addEventListener('click', hideOutcomeSub);
  ...
  document.getElementById('rotationPrev').addEventListener('click', function () {
    matchState.rotation = matchState.rotation === 1 ? 6 : matchState.rotation - 1;
    renderScoreboard();
    renderRotation();
  });
  document.getElementById('rotationNext').addEventListener('click', function () {
    matchState.rotation = (matchState.rotation % 6) + 1;
    renderScoreboard();
    renderRotation();
  });
  ...
  function updateFullscreenButtonVisibility() {
    ...
    btn.classList.remove('hidden');
    var inFs = isFullscreenActive();
    var iconExpand = btn.querySelector('.icon-expand');
    var iconExit = btn.querySelector('.icon-exit');
    if (iconExpand) iconExpand.style.display = inFs ? 'none' : '';
    if (iconExit) iconExit.style.display = inFs ? '' : 'none';
    btn.setAttribute('aria-label', inFs ? 'Volledig scherm verlaten' : 'Volledig scherm');
    btn.setAttribute('title', inFs ? 'Volledig scherm verlaten' : 'Volledig scherm');
  }

  function attachFullscreenListeners() {
    if (!isFullscreenSupported()) return;
    ['fullscreenchange', 'webkitfullscreenchange', 'MSFullscreenChange'].forEach(function (ev) {
      document.addEventListener(ev, updateFullscreenButtonVisibility);
    });
    var btn = document.getElementById('btnFullscreen');
    if (btn) btn.addEventListener('click', toggleFullscreen);
  }

  function attachFullscreenOnFirstTap() {
    if (!isFullscreenSupported()) return;
    var once = function () {
  ...
  if (doInit()) {
    ...
    function tryLoadMatch() {
      fetch('api.php?action=load&matchId=' + encodeURIComponent(matchId))
        .then(function (r) {
  ...
  if (window.scoutUtils && window.scoutUtils.addSwipeListener) {
    window.scoutUtils.addSwipeListener(document.body, {
      left: function () {
        if (matchState.matchId) {
          window.location.href = 'matchreport.php?matchId=' + encodeURIComponent(matchState.matchId);
        }
      },
      minDistance: 80
    });
